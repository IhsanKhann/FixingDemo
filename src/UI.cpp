...
void UI::initializeDummyData() {
    // This function now ONLY adds notifications for alice
    // Users and posts are loaded from files in Main.cpp
    
    User* alice = userDatabase->searchByUsername("alice");
    if (!alice) return; // Alice doesn't exist, skip notifications
